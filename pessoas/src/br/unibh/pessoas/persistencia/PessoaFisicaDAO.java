...
public class PessoaFisicaDAO implements DAO<PessoaFisica, Long>{
	...
	public PessoaFisica inserir(String nome) {
		try {
			PreparedStatement p = JDBCUtil.getConnection().prepareStatement(
							"select * from tb_pessoa_fisica "
							+ " where nome like ?");
	...
	public void update(PessoaFisica t) {
		...
		try {
			PreparedStatement p = JDBCUtil.getConnection().prepareStatement(
							...
							+ "set nome = ?, endereco = ?, telefone = ?, cpf = ?, email = ?, data_nascimento = ?, sexo = ? "
							+ "where id = ?");
	...
	public List<PessoaFisica> findAll() {
		...
		try {
			ResultSet res = JDBCUtil.getConnection().
					prepareStatement("select * from tb_pessoa_fisica").executeQuery();
			while (res.next()){
